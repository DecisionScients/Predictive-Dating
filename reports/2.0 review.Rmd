...
In this section, we will ground ourselves in the key concepts of statistical learning, supervised learning, basic linear algebra, and regression, and classification methods. 
```{python}
...
1. Perform basic vector and matrix operations such as addition,subtraction, scaling and multiplication   
2. Characterize and contrast supervised and unsupervised learning problems   
...
But first, let's align on a bit of notation.
...
Supervised learning, specifically classification will be the focus of this series. Nevertheless, we'll introduce regression problems and introduce the framework we will use to describe classification and other non-linear methods. 

## Regression Versus Classification
Supervised learning problems can be characterized in terms of their response variables. Those involving quantitative or numerical responses are labeled regression problems. Problems that seek to assign an observation to one or more classes are referred to as classification problems. 

Let's consider a quantitative response $Y$ and $p$ different predictors, $X_1, X_2, \dots X_p$.  We can write the relationship between the predictors and the response as:
...
Now for a bit of statistical decision theory that will provide the framework for developing models such as those introduced informally thus far. 

Consider $X \in \mathbb{R^p}$, a real-valued predictor vector, and $Y \in \mathbb{R}$, a real-valued response variable with joint distribution Pr$(X,Y)$. We seek to estimate a function $f(X)$ for predicting $Y$ given values of the predictor vector $X$. Since we require a loss function for penalizing errors in prediction, we shall use the most popular function, *squared* *error* *loss*:
$$L(Y, f(X))=(Y-f(X))^2.$$
...
Are we happy with that? Later in this series, we will explore methods that are far more flexibile than the linear model. For now, we examine this model-based approach in a classification context.
...
For the zero-one loss function, this simplifies to:
$$\hat{G}(x) = \operatorname{arg\,min}_{g\in G}[1 − Pr(g|X = x)],$$
and viola, our estimate for $G$:
$$\hat{G}(X)=G_k if Pr(G_k|X=x)=\operatorname{max}_{g \in G} \space Pr(g|X=x).$$
We've essentially derived the solution known as the *Bayes* *classifier* which says that we assign an observation to the most probable class, using the conditional distribution $Pr(G|X)$
