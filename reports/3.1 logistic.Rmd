...
Logistic or logit regression is a statistical model used to estimate the parameters of the sigmoid function. The sigmoid function is defined as:
$$f(x)=\frac{1}{1+e^{-x}}$$
...
ax.text(4, 0.8, r'$f(x)=\frac{1}{1+e^{-x}}$')
...
The logistic function is a special case application of the sigmoid function, and is defined as:
$$f_\theta(x)=\frac{1}{1+e^{-(\theta^TX)}},$$
where $\theta \in \mathbb{R}^{n\times (p+1)}$ is the matrix of coefficients and $x \in \mathbb{R}^{n\times (p+1)}$ is the matrix of inputs. Hence, $f_\theta(x)$ gives us the probability that our class assignment is 1, in the case of a binary classification of $Y\in[0,1]$.

In order to get our discrete 0 or 1 classification, we translate the output of $f_\theta(x)$ as follows:
$$
y = \begin{cases}
1, \space if \space f_\theta(x) \ge 0.5 \\
0, \space if \space f_\theta(x) \lt 0.5
\end{cases}
$$
Recall, the sigmoid function, $$f(x)=\frac{1}{1+e^{-x}},$$
computes an output greater than or equal to 0.5, when the input is greater than or equal zero. Concretely,  

* $x=0, e^0=1 \implies f(x)=1/2$  
* $x\rightarrow\infty, e^{-\infty}\rightarrow0\implies f(x)=1$   
* $x\rightarrow-\infty, e^{\infty}\rightarrow \infty \implies f(x)=0$   

So, if $\theta^TX \ge 0$, this means that $f_\theta(x) \ge 0.5$ .  From these statements we can now say:  
...
Let's begin with the binary classification case. We first define our cost function for logistic regression:
$$J(\theta)=\frac{1}{n}\displaystyle\sum_{i=1}^nCost(f_\theta(x_i),y_i),$$
where:
$$
\begin{matrix}
Cost(f_\theta(x),y)=-log(f_\theta(x)) & if \space y=1 \\
Cost(f_\theta(x),y)=-log(1-f_\theta(x)) & if \space y=0
\end{matrix}
$$
```{python cost}
# Prepare data
...
$$Cost(f_\theta(x),y)=-(\underbrace{ylog(f_\theta(x))}_{(1)})-(\underbrace{(1-y)log(1-f_\theta(x)))}_{(2)}$$
