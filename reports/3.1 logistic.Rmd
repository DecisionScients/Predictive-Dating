...
Next we describe the search algorithms for estimating $\theta$.

#### Gradient Descent
Our objective, at this stage, is to find $\theta$ that minimizes $J(\theta)$ For this task, we use **gradient descent**. This algorithm starts with some 'initial guess' for $\theta$, then repeatedly changes $\theta$ to make $J(\theta)$ smaller, until *hopefully* we converge to a value of $\theta$ that minimizes $J(\theta)$. Here we will examine three gradient descent algorithms: batch gradient descent, stochastic (incremental) gradient descent and Newton's method.
