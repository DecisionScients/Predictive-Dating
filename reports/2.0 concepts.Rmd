...
### Supervised Learning  
In supervised learning problems, we are given a data set with inputs or *predictors* and the output, or *response* variable. Our objective is to *learn* the relationships between the predictors and the response so that we can accurately predict responses on unseen data (*prediction*) or better understand the relationships between the predictors and response (*inference*). Supervised learning problems are categorized into *'regression'* and *'classification'* problems. Regression problems involve quantitative or continuous response variables. Real-world examples include: 
